...
cache lazy table fire_service_calls_tbl_cache as select * from demo_db.fire_service_calls_tbl;
...
select distinct year(to_date(callDate)) as year_num
from demo_db.fire_service_calls_tbl
order by year_num
...
select weekofyear(to_date(callDate)) week_year, count(*) as count
from demo_db.fire_service_calls_tbl
where year(to_date(callDate)) = 2018
group by week_year
order by count desc
